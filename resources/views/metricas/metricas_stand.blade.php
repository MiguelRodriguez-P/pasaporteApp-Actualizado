        ...
        <div class="row justify-content-center col-md-8">
            <div class="containerstars">
                <div class="caja-admin-metricasStand">
                    ...
                    <form id="formSelectAge" method="GET"
                        ...
                        <div class="form-groupmetrica mx-2">
                            ...
                            <input type="date" id="selectDate" name="fecha" value="{{ $filtroFecha }}"
                                class="mi-select">
                    ...
                    <div class="posicionfiltrar d-flex flex-wrap mx-2">
                        ...
                        @if(auth()->user()->hasRole('Empresa'))
                            <form action="{{ route('exportar.excel.stands', $idStand) }}" method="POST" class="mx-2">
                                ...
                                <button type="submit" id="filtrarexcelEvento" class="filtrarexcelEvento btn mx-2">Exportar a
                                    Excel</button>
                        @endif
                            @auth
                                @if(auth()->user()->hasRole('Administrador'))
                                    <form action="{{ route('exportar.excel.stand', $idStand) }}" method="POST">
                                        ...
                                        <input type="hidden" name="genero" value="{{ $filtroGenero }}">
                                        <input type="hidden" name="fecha" value="{{ $filtroFecha }}">
                                        <button type="submit" id="filtrarexcelEvento"
                                            class="filtrarexcelEvento btn mx-2">Exportar a Excel</button>
                                    </form>
                                @endif
                        ...
                        <table class="tabla">
                            ...
                            <tbody>
                                @foreach ($datosUsuarios as $usuario)
                                    <tr>
                                        <td data-titulo="Nombre">{{ $usuario['name'] }}
                                            <hr class="lineas">
                                        </td>
                                        <td data-titulo="E-mail">{{ $usuario['email'] }}
                                            <hr class="lineas">
                                        </td>
                                        <td data-titulo="Género">{{ $usuario['genere'] }}
                                            <hr class="lineas">
                                        </td>
                                        <td data-titulo="Edad">{{ $usuario['age'] }}
                                            <hr class="lineas">
                                        </td>
                                        <td data-titulo="Fecha">{{ $usuario['date'] }}
                                            <hr class="lineas">
                                        </td>
                                        <td data-titulo="Hora">{{ $usuario['time'] }}
                                            <hr class="lineas">
                                        </td>
                                    </tr>
                                @endforeach
                            ...
                            <table class="tabla">
                                ...
                                <tbody>
                                    @foreach ($evaluaciones as $evaluacion)
                                        <tr>
                                            <td data-titulo="Rank"> {{ $evaluacion->avg_rank }}
                                                <hr class="lineas">
                                            </td>
                                            ...
                                            <td data-titulo="Estrellas">
                                                ...
                                                <div class="calificacion Extrellascalificacion">
                                                    @for ($i = 1; $i <= 5; $i++)
                                                        @if ($i <= $promedio)
                                                            <label class="estrella-rellena"
                                                                for="estrella{{ $evaluacion->id }}{{ $i }}">&#9733;</label>
                                                        @else
                                                            <label class="estrella-vacia"
                                                                for="estrella{{ $evaluacion->id }}{{ $i }}">&#9734;</label>
                                                        @endif
                                                    @endfor
                                                </div>
                                            ...
                                            <td>
                                                @auth
                                                    @if(auth()->user()->hasRole('Empresa'))
                                                        <a href="{{ route('metricas.stand.comentarios', ['standId' => $idStand, 'criterioId' => $evaluacion->criterio->id]) }}"
                                                            class="btn leercomentario">Leer Feedback</a>
                                                    @endif
                                                @endauth
                                                @auth
                                                    @if(auth()->user()->hasRole('Evento'))
                                                        <a href="{{ route('metrica.stand.comentarios', ['standId' => $idStand, 'criterioId' => $evaluacion->criterio->id]) }}"
                                                            class="btn leercomentario">Leer Feedback</a>
                                                    @endif
                                                @endauth
                                                @auth
                                                    @if(auth()->user()->hasRole('Administrador'))
                                                        <a href="{{ route('metrica.stand.comentario', ['standId' => $idStand, 'criterioId' => $evaluacion->criterio->id]) }}"
                                                            class="btn leercomentario">Leer Feedback</a>
                                                    @endif
                                                @endauth
                                            </td>
